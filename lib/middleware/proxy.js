...
var parseProxyConfig = function(proxies) {
  ...
  Object.keys(proxies).forEach(function(proxyPath) {
    ...
    if (proxyDetails.auth) {
      proxyConfig[proxyPath].auth = proxyDetails.auth;
    }
    ...
    for (var i = 0; i < proxiesList.length; i++) {
      if (request.url.indexOf(proxiesList[i]) === 0) {
        ...
        var options = {
          target: proxiedUrl,
          ssl: {
              secure: proxyValidateSSL
          }
        }

        if (proxiedUrl.auth) {
          options.ssl.auth = proxiedUrl.auth;
        }
...
exports.create = function(/* config */ config, /* config.proxies */ proxies,
  ...
  return createProxyHandler(new httpProxy.createProxy({changeOrigin: true}),
